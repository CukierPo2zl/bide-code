...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:BIDETools/bidecode.git"]);

      const resolver = yield* RepositoryIdentityResolver;
      const identity = yield* resolver.resolve(cwd);

      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("github.com/bide/bidecode");
      expect(identity?.displayName).toBe("bide/bidecode");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("bide");
      expect(identity?.name).toBe("bidecode");
  ...
  it.effect("prefers upstream over origin when both remotes are configured", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/bidecode.git"]);
      yield* git(cwd, ["remote", "add", "upstream", "git@github.com:BIDETools/bidecode.git"]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@gitlab.com:BIDETools/platform/bidecode.git"]);

      const resolver = yield* RepositoryIdentityResolver;
      const identity = yield* resolver.resolve(cwd);

      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/bide/platform/bidecode");
      expect(identity?.displayName).toBe("bide/platform/bidecode");
      expect(identity?.owner).toBe("bide");
      expect(identity?.name).toBe("bidecode");
    }).pipe(Effect.provide(RepositoryIdentityResolverLive)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/bide/bidecode");
        expect(refreshedIdentity?.name).toBe("bidecode");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/bide/bidecode");

      yield* git(cwd, ["remote", "set-url", "origin", "git@github.com:BIDETools/bidecode-next.git"]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/bide/bidecode");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/bide/bidecode-next");
      expect(refreshedIdentity?.displayName).toBe("bide/bidecode-next");
      expect(refreshedIdentity?.name).toBe("bidecode-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),

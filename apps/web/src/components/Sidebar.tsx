  ...
  const handleProjectButtonContextMenu = useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      void (async () => {
        ...
        if (projectThreads.some((thread) => thread.archivedAt === null)) {
          toastManager.add({
            type: "warning",
...
function BideWordmark() {
  return (
    <span aria-label="BIDE" className="shrink-0 text-sm font-bold tracking-tight text-foreground">
      BIDE
    </span>
  );
}

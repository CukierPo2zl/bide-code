...
import { DEFAULT_CLIENT_SETTINGS } from "@bide/contracts/settings";
...
function createBaseServerConfig(): ServerConfig {
  return {
    ...
    cwd: "/repo/project",
    keybindingsConfigPath: "/repo/project/.bidecode-keybindings.json",
    keybindings: [],
    issues: [],
    providers: [
      {
    ...
    availableEditors: [],
    observability: {
      logsDirectoryPath: "/repo/project/.bide/logs",
...
function createMockEnvironmentApi(input: {
  ...
  return {
    ...
    agents: {} as EnvironmentApi["agents"],
...
describe("ChatView timeline estimator parity (full app)", () => {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            worktreePath: null,
            env: {
              BIDECODE_PROJECT_ROOT: "/repo/project",
            },
          });
          expect(openRequest?.env?.BIDECODE_WORKTREE_PATH).toBeUndefined();
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("bidecode:last-editor", JSON.stringify("vscodium"));
    setDraftThreadWithoutWorktree();
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              BIDECODE_PROJECT_ROOT: "/repo/project",
              BIDECODE_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
        ...
        if (body._tag === WS_METHODS.gitResolvePullRequest) {
          return {
            pullRequest: {
              ...
              url: "https://github.com/pingdotgg/bidecode/pull/1359",
              baseBranch: "main",
  ...
  it("shows the confirm archive action after clicking the archive button", async () => {
    ...
    try {
      ...
      localStorage.removeItem("bidecode:client-settings:v1");
      await mounted.cleanup();
    }
  });
        ...
        threads: createSnapshotForTargetUser({
          ...
          thread.id === THREAD_ID
            ? Object.assign({}, thread, {
                ...
                worktreePath: "/repo/.bide/worktrees/existing",
              })
            : thread,
        ),
      },
  ...
  it("keeps the wide desktop follow-up layout expanded when the footer still fits", async () => {
    const mounted = await mountChatView({
      ...
      snapshot: createSnapshotWithPlanFollowUpPrompt({
        ...
        planMarkdown:
          "# Imaginary Long-Range Plan: BIDE code Adaptive Orchestration and Safe-Delay Execution Initiative",
      }),
    });

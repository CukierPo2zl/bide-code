...
import { scopeThreadRef } from "@bide/client-runtime";
import { DEFAULT_UNIFIED_SETTINGS } from "@bide/contracts/settings";
import { normalizeModelSlug } from "@bide/shared/model";
import { createModelSelection } from "@bide/shared/model";
...
const PROVIDER_SETTINGS: readonly InstallProviderSettings[] = [
  ...
  },
  {
    ...
    serverUrlDescription: "Leave blank to let BIDE code spawn the server when needed",
...
function getProviderSummary(provider: ServerProvider | undefined) {
  ...
  if (!provider.enabled) {
    return {
      headline: "Disabled",
      detail:
        provider.message ?? "This provider is installed but disabled for new sessions in BIDE code.",
    };
  }
  ...
  return (
    <SettingsPageContainer>
      <SettingsSection title="General">
        <SettingsRow
          title="Theme"
          description="Choose how BIDE code looks across the app."
          resetAction={
            theme !== "system" ? (

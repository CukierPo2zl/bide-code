...
BIDE code has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.bide/userdata/logs/server.trace.ndjson`).
...
```bash
export BIDECODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export BIDECODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export BIDECODE_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export BIDECODE_TRACE_MIN_LEVEL=Info
export BIDECODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `BIDECODE_OTLP_*`.

macOS app bundle example:

```bash
BIDECODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
BIDECODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
BIDECODE_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/BIDE code.app/Contents/MacOS/BIDE code"
```
...
```bash
tail -f "$BIDECODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$BIDECODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$BIDECODE_HOME/userdata/logs/server.trace.ndjson"
```
...
- `BIDECODE_OTLP_TRACES_URL` was not set
...
- `BIDECODE_TRACE_FILE`: override trace file path
- `BIDECODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `BIDECODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `BIDECODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `BIDECODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `BIDECODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `BIDECODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `BIDECODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `BIDECODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `BIDECODE_OTLP_SERVICE_NAME`: service name, default `t3-server`

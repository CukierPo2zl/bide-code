...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "BIDE code (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "BIDE code (Nightly)");
  });
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                BIDECODE_DESKTOP_SKIP_BUILD: "true",
                BIDECODE_DESKTOP_KEEP_STAGE: "true",
                BIDECODE_DESKTOP_SIGNED: "true",
                BIDECODE_DESKTOP_VERBOSE: "true",
                BIDECODE_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),

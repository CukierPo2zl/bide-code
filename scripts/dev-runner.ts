...
import { NetService } from "@bide/shared/Net";
...
export const DEFAULT_BIDE_HOME = Effect.map(Effect.service(Path.Path), (path) => {
  const newDir = path.join(homedir(), ".bide");
  const legacyDir = path.join(homedir(), ".t3");
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@bide/contracts",
    "--filter=@bide/web",
    "--filter=t3",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=t3"],
  "dev:web": ["run", "dev", "--filter=@bide/web"],
  "dev:desktop": ["run", "dev", "--filter=@bide/desktop", "--filter=@bide/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("BIDECODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("BIDECODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid BIDECODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `BIDECODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric BIDECODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed BIDECODE_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_BIDE_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly bideHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(bideHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      BIDECODE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.BIDECODE_PORT = String(serverPort);
      ...
      delete output.BIDECODE_MODE;
      delete output.BIDECODE_NO_BROWSER;
      delete output.BIDECODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.BIDECODE_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.BIDECODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.BIDECODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.BIDECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.BIDECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.BIDECODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.BIDECODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.BIDECODE_MODE = "web";
      delete output.BIDECODE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read BIDECODE_PORT_OFFSET/BIDECODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.BIDECODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.BIDECODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  bideHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all BIDE code data (equivalent to BIDECODE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("BIDECODE_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to BIDECODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("BIDECODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to BIDECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("BIDECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to BIDECODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("BIDECODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to BIDECODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("BIDECODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to BIDECODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("BIDECODE_PORT")),
  ),

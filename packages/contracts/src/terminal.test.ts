...
describe("TerminalOpenInput", () => {
  ...
  it("accepts optional env overrides", () => {
    const parsed = decodeSync(TerminalOpenInput, {
      threadId: "thread-1",
      cwd: "/tmp/project",
      worktreePath: "/tmp/project/.bide/worktrees/feature-a",
      cols: 100,
      rows: 24,
      env: {
        BIDECODE_PROJECT_ROOT: "/tmp/project",
        CUSTOM_FLAG: "1",
      },
    });
    ...
    expect(parsed.worktreePath).toBe("/tmp/project/.bide/worktrees/feature-a");
  });
    ...
    expect(
      decodes(TerminalEvent, {
        ...
        snapshot: {
          threadId: "thread-1",
          terminalId: DEFAULT_TERMINAL_ID,
          cwd: "/tmp/project/.bide/worktrees/feature-a",
          worktreePath: "/tmp/project/.bide/worktrees/feature-a",
          status: "running",
          pid: 1234,
          history: "",
